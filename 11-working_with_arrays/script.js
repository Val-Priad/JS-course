...
const getFormattedDateForMovements = (account, date) => {
...
const getFormattedCurrency = (account, value) => new Intl.NumberFormat(account.locale, {
    style: "currency",
...
const displayMovements = (account, sort = false) => {
    ...
    movements.forEach((mov, idx) => {
        let movementDate = new Date(mov.date);
...
const calcSetIncomes = (account) => {
    labelSumIn.textContent = getFormattedCurrency(account, account.movements.filter((m) => m > 0).reduce((acc, m) => m + acc));
};
const calcSetOuts = (account) => {
    labelSumOut.textContent = getFormattedCurrency(account, account.movements.filter((m) => m < 0).reduce((acc, m) => m + acc));
};
const calcSetInterest = (account) => {
    labelSumInterest.textContent = getFormattedCurrency(account, account.movements
        .filter((dep) => dep > 0)
